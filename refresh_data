...
CONFIG_FILE = Path(__file__).parent / "netlify_config.json"

def load_or_setup_netlify():
    if CONFIG_FILE.exists():
        cfg = json.loads(CONFIG_FILE.read_text())
        print(f"Netlify: {cfg.get('site_name','')} → {cfg.get('site_url','')}")
        return cfg
    print("\n" + "="*50)
    print("NETLIFY SETUP — one time only")
    print("="*50)
    print("\n1. netlify.com → your site → Site settings → copy 'Site ID'")
    print("2. netlify.com → profile photo → User settings → Applications")
    print("   → New access token → name it 'MFD' → copy token\n")
    site_id = input("Paste Site ID: ").strip()
    token   = input("Paste Token:   ").strip()
    r = requests.get(f"https://api.netlify.com/api/v1/sites/{site_id}",
                     headers={"Authorization": f"Bearer {token}"}, timeout=10)
    if r.status_code != 200:
        print(f"❌ Connection failed ({r.status_code}). Check credentials."); sys.exit(1)
    info = r.json()
    cfg  = {"site_id": site_id, "token": token,
            "site_url": info.get("ssl_url",""), "site_name": info.get("name","")}
    CONFIG_FILE.write_text(json.dumps(cfg, indent=2))
    print(f"✅ Connected: {cfg['site_name']}")
    return cfg

def upload_to_netlify(cfg, data_str):
    r = requests.put(
        f"https://api.netlify.com/api/v1/sites/{cfg['site_id']}/files/data.json",
...
def main():
    ...
    print(f"\n  data.json saved — {len(data_str)//1024} KB")

    print("\nConnecting to Netlify...")
    cfg = load_or_setup_netlify()
    if upload_to_netlify(cfg, data_str):
        print(f"  ✅ Uploaded successfully")
    else:
        print(f"  ⚠  Upload failed — data.json saved locally, upload manually")
    ...
    print(f"\n  Share this URL with managers:")
    print(f"  {cfg.get('site_url', 'your Netlify URL')}")
    print(f"\n  Run this script again monthly to refresh data.")
